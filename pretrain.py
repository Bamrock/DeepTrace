...
class Config(object):
    def __init__(self):
        ...
        self.mlm = True
        self.mlm_probability = 0.05
        self.logging_steps = 500
        self.save_steps = 500
        self.save_total_limit = 20
...
def get_disease_tokens_mask(disease_vocab, squence):
    disease_index = [0] * len(squence)
    for i in range(len(squence)):
        if squence[i] in disease_vocab:
            disease_index[i] = 1
...
def mask_tokens(inputs: torch.Tensor, tokenizer: PreTrainedTokenizer, args) -> Tuple[torch.Tensor, torch.Tensor]:
    ...
    probability_matrix = torch.full(labels.shape, args.mlm_probability)  # shape与labels相同，值为mlm_probability的tenser
    special_tokens_mask = [
        tokenizer.get_special_tokens_mask(val, already_has_special_tokens=True) for val in labels.tolist()
    ]
    disease_tokens_mask = [
        get_disease_tokens_mask(args.disease_vocab, val) for val in labels.tolist()
    ]
    probability_matrix.masked_fill_(torch.tensor(special_tokens_mask, dtype=torch.bool),
                                    value=0.0)  # 用value填充tensor中与mask中值为1位置相对应的元素。mask的形状必须与要填充的tensor形状一致。
    probability_matrix.masked_fill_(torch.tensor(disease_tokens_mask, dtype=torch.bool), value=1.0)
    if tokenizer._pad_token is not None:
        padding_mask = labels.eq(tokenizer.pad_token_id)
        probability_matrix.masked_fill_(padding_mask, value=0.0)
    masked_indices = torch.bernoulli(probability_matrix).bool()  # 生成值为0或1的tensor，生成1的概率是mlm_probability
...
def train(args, train_dataset, model: PreTrainedModel, tokenizer: PreTrainedTokenizer) -> Tuple[int, float]:
    ...
    for _ in train_iterator:
        ...
        for step, batch in enumerate(epoch_iterator):
            ...
            model.train()
            #### saving initinization model
            # Save model init checkpoint
            checkpoint_prefix = "checkpoint"
            output_dir_init = os.path.join(args.output_dir_init, "{}-{}".format(checkpoint_prefix, global_step))
            os.makedirs(output_dir_init, exist_ok=True)
            model_to_save = (
                model.module if hasattr(model, "module") else model
            )  # Take care of distributed/parallel training
            model_to_save.save_pretrained(output_dir_init)
            tokenizer.save_pretrained(output_dir_init)

            torch.save(args, os.path.join(output_dir_init, "training_args.bin"))
            logger.info("Saving model checkpoint to %s", output_dir_init)

            _rotate_checkpoints(args, checkpoint_prefix)

            torch.save(optimizer.state_dict(), os.path.join(output_dir_init, "optimizer.pt"))
            torch.save(scheduler.state_dict(), os.path.join(output_dir_init, "scheduler.pt"))
            logger.info("Saving optimizer and scheduler states to %s", output_dir_init)
            print("finish save init model")
...
def evaluate(args, model: PreTrainedModel, tokenizer: PreTrainedTokenizer, prefix="") -> Dict:
    ...
    for batch in tqdm(eval_dataloader, desc="Evaluating"):
        ...
        inputs = inputs.to(args.device)
        labels = labels.to(args.device)
        total_m += (labels != -100).sum().item()

        with torch.no_grad():
            outputs = model(inputs, masked_lm_labels=labels) if args.mlm else model(inputs, labels=labels)
            correct += outputs[1].argmax(dim=-1).eq(labels).sum().item()
            lm_loss = outputs[0]
            eval_loss += lm_loss.mean().item()
        nb_eval_steps += 1
    eval_loss = eval_loss / nb_eval_steps
    perplexity = torch.exp(torch.tensor(eval_loss))
    accuracy_m = correct / total_m
    result = {"perplexity": perplexity, "accuracy": accuracy_m}
    output_eval_file = os.path.join(eval_output_dir, prefix, "eval_results.txt")
    with open(output_eval_file, "a") as writer:
        logger.info("***** Eval results {} *****".format(prefix))
        for key in sorted(result.keys()):
            logger.info("  %s = %s", key, str(result[key]))
            # writer.write(key + '\t' + str(float(result[key])) + "\n")
            writer.write("%s = %s\n" % (key, str(result[key])))
...
def main():
    ...
    args = Config()
    args.local_rank = args_parser.local_rank
    ...
    vocab_file = open(args.output_dir + '/vocab.txt', 'r', encoding='utf8').readlines()
    vocab = list(map(lambda x: x.strip(), vocab_file))
    disease_vocab = []
    for i in vocab:
        if 'M' in i or 'L' in i:
            if i not in ['[CLS]', '[MASK]']:
                disease_vocab.append(vocab.index(i))
    args.disease_vocab = disease_vocab
