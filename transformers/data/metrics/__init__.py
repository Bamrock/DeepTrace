...
if _has_sklearn:
    ...
    def acc_spec_sens_auc_f1(preds, labels, probs):
        acc = simple_accuracy(preds, labels)
        cm = confusion_matrix(labels, preds)
        if len(np.unique(labels))==3:
            specificity = (cm[0][0]+cm[2][2]) / (cm[0][0]+cm[0][1]+cm[0][2]+cm[2][0]+cm[2][1]+cm[2][2])
            sensitivity = cm[1][1] / (cm[1][0]+cm[1][1]+cm[1][2])
            acc_HD = cm[0][0] / (cm[0][0]+cm[0][1]+cm[0][2])
            acc_healthy = cm[2][2] / (cm[2][0]+cm[2][1]+cm[2][2])
            return {
                "acc": acc,
                "spec": specificity,
                "sens": sensitivity,
    ...
    def glue_compute_metrics(task_name, preds, labels, probs=None):
        ...
        if task_name == "cola":
            ...
            # return {"acc": simple_accuracy(preds, labels)}
            return acc_spec_sens_auc_f1(preds, labels, probs)
