...
def glue_convert_pred_examples_to_features(
    examples,
    tokenizer,
    max_length=512,
    task=None,
    ...
    for (ex_index, example) in enumerate(examples):
        ...
        if output_mode == "classification":
            label = label_map[example.label]
            read_id = int(example.read_id)
            DMR = example.DMR
            sample_id = example.sample_id
            group = int(example.group)
            read_merged = int(example.read_merged)
        elif output_mode == "regression":
            label = float(example.label)
        else:
            raise KeyError(output_mode)
        ...
        features.append(
            InputFeatures(
                input_ids=input_ids, attention_mask=attention_mask, token_type_ids=token_type_ids, label=label,
                read_id=read_id, DMR=DMR, sample_id=sample_id, group=group, read_merged=read_merged
            )
        )
    if is_tf_available() and is_tf_dataset:
        def gen():
...
class DnaPromProcessor(DataProcessor):
    ...
    def get_pred_examples(self, data_dir):
        return self._create_pred_examples(self._read_tsv(os.path.join(data_dir, "pred.tsv")), "pred")
        # return self._create_examples(self._read_tsv(os.path.join(data_dir, "pred.tsv")), "pred")
    ...
    def _create_pred_examples(self, lines, set_type):
        ...
        for (i, line) in enumerate(lines):
            ...
            text_a = line[0]
            label = line[1]
            read_id, DMR, sample_id, group, read_merged = line[2], line[3], line[4], line[5], line[6]
            # print("&&&&&&&&&&&&&&&&&&&&&&&&&&&&&")
